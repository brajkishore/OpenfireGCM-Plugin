...
public class GCMUpdater implements Plugin, PacketInterceptor {

    private static final Logger Log = LoggerFactory.getLogger(GCMUpdater.class);
    ...
    public void initializePlugin(PluginManager pManager, File pluginDirectory) {
        debug = JiveGlobals.getBooleanProperty("plugin.gcm.updater.debug", false);
        if (debug) {
        ...
        url = getProperty("plugin.gcm.updater.uri", "");
        token = getProperty("plugin.gcm.updater.token", UUID.randomUUID().toString());
            ...
            try {
                ...
                if (!available) {
                    ...
                    WebTarget target = client.target(url)
                    		...
                    		.queryParam("message", URLEncoder.encode(msg.getBody(), "UTF-8"));
                    ...
                    System.out.println("got response status url='{}' status='{}'"+target+" "+response.getStatus());
                    if (debug) {
                        try {
